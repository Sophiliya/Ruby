class CargoTrain
  ...
  # метод hitch_or_detach добавила в private чтобы не писать одинаковые два метода
  # и чтобы было два разных метода, чтобы юзеру не пришлось вводить направление движения
  
  def hitch_or_detach
    return unless @current_speed == 0

    if @action == 'hitch' && @wagon.type == 'Cargo'
      @wagons << @wagon
    elsif @action == 'detach'
      @wagons.delete(@wagon)
    end
